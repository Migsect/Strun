...
public class ParseLogger
{
	...
	/**Returns the instance of the parse logger
	 * This will construct a new parse logger if one doesn't already eist
	 * 
	...
	public enum LogState
	{
		...
		LogState(String tag){this.tag = tag;}
	...
	private boolean showAllChannels(){return this.channels.contains(ALL_CHANNELS);}
	private boolean hasActivatedChannel(String[] channels)
	{
		for(String c : channels) if(this.channels.contains(c.toUpperCase())) return true;
		return false;
	}
	...
	{
		if(!this.log_state.contains(state)) return; // The log state has been activated
		if(!this.hasActivatedChannel(channels) && !this.showAllChannels()) return; // The channel is activated.
