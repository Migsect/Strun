...
public abstract class NativeExpression implements Expression
{
  ...
  /**
   * Will return the frunction that is one level up from the current
   * environment. If the environment is null, then this will do little to stop
  ...
  /**
   * Will return the frunction with the symbol. If it doesn't exist we will
   * print stack trace and then return null.
   * 
   * @param symbol
   *          The symbol to check the environment for
   ...
   * 
   * @param key
   *          The key for the dynamic symbol
   * @param environment
   *          The environment this is defined in
   * @param input
   *          The inputs for the inner method binding
   * @param types
   *          The types for the inner method binding
   * @param condition
   *          The condition for the inner method binding.
   * @return A symbol binding.
   */
  public SymbolBinding getAsBinding(String key, Container environment,
      String[] input, String[] types, Expression condition)
  {
    Frunction method_holder = new DynamicFrunction(environment);

    MethodBinding method = new DynamicMethodBinding(environment, input, types,
        condition, this);
    try
    {
