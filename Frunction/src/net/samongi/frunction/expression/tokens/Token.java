...
public interface Token
{
  ...
  public static Token.Type[] arrayToTypeArray(Token[] tokens)
  {
    ...
    for(int i = 0; i < tokens.length; i++) type_array[i] = tokens[i].getType();
    return type_array;
  }
  
  /**Parses the string trying to find tokens
   * If multiple tokens are found, it will return a group token.
   * 
   * @param source
   * @return
  ...
  public static GroupToken parseTokens(String source) throws TokenException
  {
	 ...
	 * This can be used with just the class but this is more official
	 * 
	 * @return
	 */
