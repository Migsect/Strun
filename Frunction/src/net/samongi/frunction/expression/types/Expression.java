...
public interface Expression
{
   ...
   *          The tokens to parse
   * @param environment
   *          The environment the tokens are being parsed in
   * @return An expression parsed from the tokens
   * @throws TokenException
   */
  public static Expression parseTokens(Token[] tokens, Container environment)
      throws TokenException
  {
    if(DEBUG)
      System.out.println("  Expression Parser: Parsing through '"
          + tokens.length + "' tokens.");
    if(DEBUG) System.out.println("    Token Types:");
    if(DEBUG) for(Token t : tokens)
      System.out.println("     - " + t.getType().toString());

    if(environment == null)
      System.out.println("  Expression Parser: Environment is null!");
    ...
    while(i < tokens.length)
    {
      ...
      {
        System.out
            .println("  Expression Parser: left expression null on not first loop.");
        return null;
      }
      ...
      if(t.getType().equals(Token.Type.SYMBOL))
      {
        if(i != 0) // Needs to be the first token if this case happens
        {
          if(DEBUG)
            System.out
                .println("  Issue in Expression Parser: Symbol found it was not first");
          return null;
        }
        ContainerAccessorExpression expr = new ContainerAccessorExpression(
            environment, (SymbolToken) t);
      ...
      else if(t.getType().equals(Token.Type.ACCESSOR))
      {
        ...
        FrunctionAccessorExpression expr = new FrunctionAccessorExpression(
            left_expr, (SymbolToken) r_t);
        left_expr = expr; // Setting it to be the left expression
        i += 2;
      }
      ...
      else
      {
        // TODO unknown token
        if(DEBUG)
          System.out.println("  Issue in Expression Parser: Unknown Token");
        return null;
      }
    ...
    if(left_expr == null)
    {
      if(DEBUG)
        System.out
            .println("  Issue in Expression Parser: Left expression was null.");
      return null;
    }
